...
abstract class Person(val name: String, val surname: String, val birthDate: BirthDate){
  // zwraca true lub false w zależności od tego czy osoba jest pełnoletnia,
  // dla uproszczenia można przyjąć, że zawsze jest 16 kwietnia 2018,
  // a pełnoletność uzyskuje się w dzień urodzin o północy
...
class Student(name: String, surname: String, birthDate: BirthDate, val university: String, val subject: String, val year: Int) extends Person(name, surname, birthDate){
  // Zwraca Both, University, Subject lub None w zależności od tego czy podane jako parametry uniwersystet i kierunek
  // zgadzają się z tymi gdzie studiuje student
...
class Employee(name: String, surname: String, birthDate: BirthDate, val company: String, val position: String, val salary: Int) extends Person(name, surname, birthDate){
  ...
  // Zwraca tekst "Equal", "Lower" albo "Higher" w zależności od podanej parametrem kwoty i pensji pracownika
  ...
  // Zwraca true jeśli nazwa pozycji pracownika zawiera:
...
// Do uruchomienia testów wystarczy kliknąć prawym na nazwie klasy i wybrać "Run PersonTest"
// Teoretycznie można uruchamiać pojedyncze testy klikając prawym na poszczególnym przypadku
// ale IntelliJ IDEA średnio sobie z tym radzi i czasami odpala kompletnie inne przypadki testowe
